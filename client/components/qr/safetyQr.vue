<template>
  <div class="row justify-center content-start qrCreateContainer">
    <div class="col-12 row justify-center titleBar">
      <span>{{ $t("safetyQR") }}</span>
    </div>
    <div class="col-12 row content-start">
      <div class="col-12 divisionLine"></div>
      <div class="col-12 row inputOptions items-center">
        <span class="col-auto" v-show="false">{{ $t("safetyQR text")[0] }}</span>
        <input class="col" v-model="pack" disabled v-show="false" />
        <div class="col-12 divisionLine"></div>
        <div class="col-12 row inputOptions items-center"></div>
        <span class="col-auto">{{ $t("safetyQR text")[1] }}</span>
        <input class="col" v-model="title" :placeholder='$t("safetyQR text")[4]' />
        <div class="col-12 divisionLine"></div>
        <div class="col-12 row inputOptions items-center" v-show="false"></div>
        <span class="col-auto" v-show="false">{{ $t("safetyQR text")[2] }}</span>
        <input class="col" v-model="type" v-show="false" />
        <div class="col-12 divisionLine" v-show="false"></div>
        <div class="col-12 row inputOptions items-center"></div>
        <span class="col-auto">{{ $t("safetyQR text")[3] }}</span>
        <input class="col" v-model="url" :placeholder='$t("safetyQR text")[5]' />
        <div class="col-12 divisionLine"></div>
      </div>
  ...
  methods: {
    ...
    QRCreateBtnClick() {
      if (
        ...
        this.splitExtension = this.splitUrl[this.splitUrl.length - 1].split(".")
        this.type = this.splitExtension[this.splitExtension.length - 1]

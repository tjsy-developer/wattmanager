  ...
  .col-6.column.items-center.accountRight
    ...
    .col.row.items-center
      .maxWidth(v-if="!isCreateQR")
        ...
        input(:placeholder="$t('account')[14]", v-model="password", type="password").input
        ...
        input(id="enterpriseName" :placeholder='$t("enName")', v-model="enName" @keyup.enter="enterpriseCheckBtnClick").enInput
        button(@click="enterpriseCheckBtnClick").nameChkBtn.col-auto {{ $t("account")[18] }}
        ...
        transition(name="qrcode", mode="out-in")
          .emptyQrCode(v-if="!qrCodeImg", key="emptyQrCode").row.justify-center
            .col-12.row.justify-center
              .emptyQrCodeBox
            span {{ $t("account")[0] }}
          .loadQrCode(v-else, key="loadQrCode").row.jusdutify-center
            .col-12.row.justify-center

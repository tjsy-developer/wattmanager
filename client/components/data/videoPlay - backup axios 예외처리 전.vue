  ...
  .row(v-if="selected").justify-center.videoPlay
    .col-auto.row
      .col.column.left
        ...
        video(v-else :src="selected.video", :poster="selected.thumbnail", autoplay, controls, controlsList="nodownload" ).col-auto
    ...
    return {
      ...
      rightListInfiniteScrollCompData: {
        ...
        async setRightListData(getListData) {
          ...
          //   // 데이터가 없더라도 영상은 보여줄 수 있도록 rightBarListEnd를 1로 변경한다.
          //   if (self.rightBarListEnd === 0) {
          //     console.log("getListData Empty ! > rightBarListEnd")
          ...
          for (let i = 0; i < getListData.length; i++) {
            ...
            await self.axios
              ...
              .catch(function(error) {
                console.log("videoplay.vue setRightListData error : ", error)
              })
            // 마지막 일 경우 체크
